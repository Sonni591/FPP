 ...
 *
 */
package de.oth.smplsp.util.jfreechartfx.interaction;

import javafx.scene.input.MouseEvent;
import javafx.scene.input.ScrollEvent;
import de.oth.smplsp.util.jfreechartfx.ChartCanvas;

/**
 * The interface for a mouse handler, which is an object that listens for mouse
 * events on a {@link ChartCanvas} and performs a function in response to those
 * events (typical functions include tooltip display, drag zooming, mouse wheel
 * zooming and panning). A handler can be registered with the
 ...
 * events (it can be reselected again on subsequent mouse pressed events). The
 * auxiliary handlers are always called to respond to mouse events, but after
 * the live handler has dealt with the event first. Auxiliary handlers should
 * not perform tasks that could interfere with the live handler.
 * 
 * <p>
...
public interface MouseHandlerFX {
    ...
    /**
     * Returns {@code true} if the mouse handler is enabled, and {@code false}
     * if it is disabled.
     * 
     * @return A boolean.
     */
    ...
    /**
     * Returns {@code true} if the specified mouse event has modifier keys that
     * match this handler.
     * 
     * @param e
     *            the mouse event ({@code null} not permitted).
     * 
     * @return A boolean.
     */
     ...
     * 
     * @param canvas
     *            the canvas ({@code null} not permitted).
     * @param e
     *            the event ({@code null} not permitted).
     */
