...
/**
 * Handles drag zooming of charts on a {@link ChartCanvas}. This handler should
 * be configured with the required modifier keys and installed as a live handler
 * (not an auxiliary handler). This handler only works for a <b>ChartCanvas</b>
 * that is embedded in a {@link ChartViewer}, since it relies on the
 * <b>ChartViewer</b> for drawing the zoom rectangle.
 * 
 * <p>
...
public class ZoomHandlerFX extends AbstractMouseHandlerFX {
     ...
     * 
     * @param canvas
     *            the JavaFX canvas (<code>null</code> not permitted).
     * @param e
    ...
    /**
     * Handles a mouse dragged event by updating the zoom rectangle displayed in
     * the ChartViewer.
     * 
     * @param canvas
     *            the JavaFX canvas (<code>null</code> not permitted).
	...
	boolean zoomTrigger1 = hZoom
		&& Math.abs(e.getX() - this.startPoint.getX()) >= 10;
	boolean zoomTrigger2 = vZoom
		&& Math.abs(e.getY() - this.startPoint.getY()) >= 10;
	if (zoomTrigger1 || zoomTrigger2) {
		...
		PlotRenderingInfo info = this.viewer.getRenderingInfo()
			.getPlotInfo();
